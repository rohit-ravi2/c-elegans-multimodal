...
## 1-paragraph abstract (v3, tautology-corrected)

**A stringent test of wiring prediction:** We restrict analysis to C. elegans neuron pairs that physically touch in the nerve ring (from Brittin/Zhen EM adjacency), because pairs without contact cannot form synapses and including them inflates predictive AUC by a tautology. Using Witvliet et al. 2020's developmental series (L1 through adult) paired with the CeNGEN L4 neuron-class expression atlas, we ask: **among neuron pairs that physically touch, which ones form chemical synapses across developmental transitions?** On N = 22,090 contacting candidate pairs (1,504 arrivals), a three-layer nested model achieves **AUC 0.823 [95% CI 0.812–0.833]**: contact-area alone predicts at AUC 0.690 [0.675, 0.703]; adding chemical-graph topology (shared output partners, triadic closure) adds **Δ = +0.066 [+0.058, +0.075]** to AUC 0.756; adding CeNGEN class-level gene expression adds a further **Δ = +0.067 [+0.057, +0.077]** to AUC 0.823. All three layers contribute independently, and all 95% CIs on the deltas exclude zero under paired bootstrap (1,000 resamples). The mechanism hierarchy — contact-as-opportunity, topology-as-specificity, transcription-as-refinement — maps cleanly to developmental biology. The rule is chemical-synapse-specific: electrical (gap junction) arrival is at chance (AUC 0.60), and the peptide-wireless signaling graph is structurally distinct from the synaptic graph (Jaccard 0.045), with 5 textbook state-integrator neurons (RIM, RMG, RIG) identified as multiplex hubs. Static gene-motif correlations at class level (N=84) universally fail BH-FDR correction, supporting the view that gene expression is not a primary architect of *C. elegans* wiring but a modulatory refinement within the contact/topology-constrained space.
