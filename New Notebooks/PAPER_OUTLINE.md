# Paper 1 — Draft Outline (v4, post-Nb 24–27)

**Working title:** Two orthogonal organizational principles — topology for wiring, identity for neuromodulation — jointly explain the *C. elegans* multilayered signaling atlas.
...
## 1-paragraph abstract (v4, with channel-selection asymmetry)

The *C. elegans* nervous system uses multiple signaling modalities — chemical synapses, gap junctions, and peptide/GPCR wireless broadcasting — that form structurally distinct networks on the same set of neurons. We show that these modalities follow **orthogonal organizational principles**: chemical wiring is placed according to **network position**, while peptide signaling is placed according to **cell identity**. Restricting to N = 10,060 neuron pairs in direct physical contact in the nerve ring, a 4-way channel classification (neither / synapse-only / peptide-only / both) reveals that local topology and contact ALONE predict synaptic placement at AUC 0.857 but peptide placement at only AUC 0.600 (Nb 27). The peptide-placement deficit is recovered by adding cell-identity features: adding gene-expression PCA lifts macro-AUC from 0.749 to 0.867, and all four classical monoamine pre-neuron classes (Tyramine, Serotonin, Dopamine, Octopamine) are 3.5–9.5× enriched in peptide-only contacting pairs (Nb 25, p as extreme as 7×10⁻²⁹). In parallel, a developmental edge-arrival analysis on Witvliet 2020 stages L1→adult shows that among N = 22,090 contacting pairs, a three-layer additive model (contact + topology + genes) predicts chemical-synapse arrival at AUC 0.823 [95% CI 0.812–0.833], with topology adding +0.066 AUC and gene PCA adding +0.067 AUC (Nb 24), each delta's CI strictly excluding zero. A distilled 8-feature interpretable model preserves 0.791 AUC with only 4 stably-important features: log-contact, two gene PCs, and triadic closure (Nb 26). Static gene-motif correlations at class level (N = 84) universally fail BH-FDR correction, and signed-motif analysis reveals an enriched (−,+,−) dual-inhibition gate at z = +28 (Nb 13). Five textbook state-integrator neurons (RIM L/R, RMG L/R, RIGR) are identified *de novo* as multiplex hubs bridging the synaptic and peptide layers (Nb 23). Together these results reframe *C. elegans* neural wiring as a multi-layer control system in which different signaling modalities follow distinct, complementary organizational grammars.
